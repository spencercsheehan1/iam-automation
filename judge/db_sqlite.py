...
Local-dev / sample-mode backend — see db.py for backend selection.
Not used in production (JUDGE_DB_BACKEND=dynamodb): a container's local
disk doesn't survive restarts/redeploys and doesn't share state across
instances, which would silently break "inspect previous results" as an
audit control. See db_dynamo.py for the production backend.
